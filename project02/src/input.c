...
void input(Laplace* solver, const char* input_file) {
    ...
    if(strcmp(solver->iter_method, "Jacobi") != 0 && strcmp(solver->iter_method, "Gauss-Seidel") != 0 && strcmp(solver->iter_method, "PETSC") != 0) {
       printf("Input Error! The iter_method must be \"Jacobi\" or \"Gauss-Seidel\" or \"PETSC\". Please try again.\n");
       exit(1);
    }
    ...
    if(grvy_input_fread_char("solver/output_file", &(solver->output_file))) {
        printf("--> %-25s = %s\n","output file", solver->output_file);
    }
